...
import Toast from '../(components)/elements/ToastNotification'
...
const ReservationList : React.FC<ReservationListProps> = ({
  ...
  // Ajoutez cet état local pour stocker les réservations mises à jour
  ...
  // Mettre à jour l'état local quand les réservations changent
  useEffect(() => {
  ...
  // Fonction pour visualiser le PDF d'un contrat existant
  const viewContract = (contractUrl: string) => {
    if (!contractUrl) {
      updateState({ typeToast: 'error', contentToast: 'Le fichier du contrat n\'est pas disponible.' });
      return;
    }
    
    // Si l'URL est relative, la préfixer avec l'URL de base de l'API
    if (contractUrl.startsWith('/')) {
      const baseUrl = process.env.NEXT_PUBLIC_API_URL_SAP || '';
      contractUrl = `${baseUrl}${contractUrl}`;
    }
    ...
    fetch(contractUrl, { method: 'HEAD' })
      .then(response => {
        if (response.ok) {
          ...
          updateState({ typeToast: 'error', contentToast: 'Le fichier du contrat n\'existe pas.' });
        }
      })
      .catch(error => {
        console.error("Erreur lors de la vérification du contrat:", error);
        updateState({ typeToast: 'error', contentToast: 'Impossible d\'accéder au fichier du contrat.' });
      });
  }
  ...
  // Utiliser localReservations pour la pagination au lieu de reservations
  const totalPages = Math.ceil(localReservations.length / itemsPerPage)
  const startIndex = (currentPage - 1) * itemsPerPage
  const paginatedData = localReservations.slice(
    startIndex,
  ...
  const onSubmit = (data: any) => {
    ...
    const dataFinal = {
      ...data,
      state: valueStatus || state.defaultValues.state
    }

    // Mettre à jour localement le statut avant d'envoyer au serveur
    const updatedReservations = localReservations.map((reservation: any) => {
      if (reservation.id === state.defaultValues.id) {
        return { ...reservation, state: dataFinal.state };
      }
      return reservation;
    });
  ...
  const toggleUpdateStatus = (id: any) => updateState({ 
    isOpenUppdateStatus: true, 
    formTitle: "Modifier Status", 
    defaultValues: reservations.find((r: any) => r.id === id) 
  })
    ...
    <div>
      ...
      }
      {
        ...
        <Form
          ...
          setStatus={setValueStatus}
          status={valueStatus || state.defaultValues.state}
              ...
              {paginatedData.map((d: any, index: any) => (
                ...
                >
                          ...
                          >
                            <div className='bg-white p-2 shadow-xl mx-4 sm:mx-auto md:mx-auto lg:mx-auto xl:mx-auto mt-16 rounded-lg sm:max-w-sm md:max-w-sm lg:max-w-sm xl:max-w-sm max-w-2xl text-gray-900'>
                              ...
                              <div className="flex items-center justify-between p-3 bg-[#85C1E9]/10 border border-[#85C1E9]/30 mt-3 rounded-xl">
                                ...
                                {(() => {
                                  const payment: any = paiments.find((p: any) => p.reservation_id === d.id);
                      ...
                      className={`bg-green-100 px-3 py-1 rounded-full ${poppins.className} ${d.state === 'confirmed'
                        ? 'dark:bg-green-700 text-green-700 dark:text-green-100' : d.state === 'pending'
                          ? 'bg-orange-200 text-orange-500' : d.state === 'completed'
                            ? 'bg-blue-200 text-blue-500' : 'bg-red-100 text-red-500'
                  ...
                  <td className={`px-4 py-3 text-sm ${poppins.className}`}>
                    {d.contract_url ? (
                      <Button
                        onClick={() => viewContract(d.contract_url)}
                        variant='outline'
                        className={`font-bold text-lg text-[#1b4569] hover:bg-blue-50 ${quicksand.className}`}
                      >
